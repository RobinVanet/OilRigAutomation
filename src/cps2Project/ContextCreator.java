...
public class ContextCreator implements ContextBuilder<Agent> {
	...
	public Context<Agent> build(Context<Agent> context) {
		...
		for (int i = 0; i < nbSensor - 1; i++) {
			...
			double dangerTemp = 100 + (int) (Math.random() * ((125 - 100) + 1)); // random
																					// value
																					// from
																					// 100
																					// to
																					// 125
			double criticalTemp = 125 + (int) (Math.random() * ((150 - 125) + 1)); // random
																					// value
																					// from
																					// 125
																					// to
		...
		double dangerTemp = 100 + (int) (Math.random() * ((125 - 100) + 1));
		// random value from 100 to 125

		double criticalTemp = 125 + (int) (Math.random() * ((150 - 125) + 1));
	...
	public double getSpeed() {
		...
		speed = K * getWOBFactor(weightOnBit, holeDiameter) * getROPFactor(rotationsPerMinute, hardFormation)
				* getFlowFactor();
